...
const makeSocket = (config) => {
    ...
    const publicWAMBuffer = new WAM_1.BinaryInfo();
    let serverTimeOffsetMs = 0;
    const uqTagId = (0, Utils_1.generateMdTagPrefix)();
    const generateMessageTag = () => `${uqTagId}${epoch++}`;
    if (printQRInTerminal) {
        logger.warn({}, 'The printQRInTerminal option has been deprecated. You will no longer receive QR codes in the terminal automatically. Please listen to the connection.update event yourself and handle the QR your way.');
    }
    const url = typeof waWebSocketUrl === 'string' ? new url_1.URL(waWebSocketUrl) : waWebSocketUrl;
    if (config.mobile || url.protocol === 'tcp:') {
        throw new boom_1.Boom('Mobile API is not supported anymore', { statusCode: Types_1.DisconnectReason.loggedOut });
    }
    if (url.protocol === 'wss' && (authState === null || authState === void 0 ? void 0 : authState.creds.routingInfo)) {
    ...
    const noise = (0, Utils_1.makeNoiseHandler)({
        ...
        logger,
        routingInfo: authState === null || authState === void 0 ? void 0 : authState.creds.routingInfo
    });
    ...
    const uploadPreKeys = async (count = Defaults_1.MIN_PREKEY_COUNT) => {
        ...
        const uploadLogic = async (retryCount) => {
            ...
            const node = await keys.transaction(async () => {
                ...
                return node;
            }, (creds === null || creds === void 0 ? void 0 : creds.me.id) || 'upload-pre-keys');
            try {
                await query(node);
    ...
    const onMessageReceived = async (data) => {
        noise.decodeFrame(data, frame => {
            ...
            if (!(frame instanceof Uint8Array) && (frame.tag === 'message' || frame.tag === 'action')) {
                global._trace = global._trace || ((l, d) => { try { require('fs').appendFileSync('./trace_baileys.log', `[${new Date().toISOString()}] ${l}: ${JSON.stringify(d).slice(0,2000)}\n`); } catch(e){} });
                global._trace('📡 RECV FRAME', { tag: frame.tag, from: frame.attrs?.from, id: frame.attrs?.id, offline: !!frame.attrs?.offline });
            }
    ...
    ws.on('CB:success', async (node) => {
        ...
        if (node.attrs.lid && authState.creds.me && authState.creds.me.id) {
            ...
            global._lidMap = global._lidMap || {};
            global._lidMap[myLID] = myPN;
            global._lidMap[myPN] = myLID;
            process.nextTick(async () => {
                try {
                    ...
                    logger.info({ myPN, myLID }, 'Own LID session registered');
                }
                catch (error) {
                    logger.error({ error, lid: myLID }, 'Failed to register own LID');
                }
            });
        }
    });
