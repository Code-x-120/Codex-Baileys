...
const makeMessagesRecvSocket = (config) => {
    ...
    const handleMessage = async (node) => {
        ...
        global._trace('📩 handleMessage', { from: node.attrs.from, id: node.attrs.id, enc: (0, WABinary_1.getBinaryNodeChild)(node, 'enc')?.attrs?.type });
        ...
        if ((0, WABinary_1.getBinaryNodeChild)(node, 'unavailable') && !encNode) {
            const unavailableType = (0, WABinary_1.getBinaryNodeChild)(node, 'unavailable')?.attrs?.type;
            if (unavailableType === 'view_once_unavailable_fanout') {
                logger.debug({ key: node.attrs.key }, 'skipping placeholder resend for view_once_unavailable_fanout');
                await sendMessageAck(node);
                return;
            }
            await sendMessageAck(node);
        ...
        if (((_c = (_b = msg.message) === null || _b === void 0 ? void 0 : _b.protocolMessage) === null || _c === void 0 ? void 0 : _c.type) === WAProto_1.proto.Message.ProtocolMessage.Type.SHARE_PHONE_NUMBER && node.attrs.sender_pn) {
            ...
            global._lidMap[node.attrs.from] = node.attrs.sender_pn;
            global._lidMap[node.attrs.sender_pn] = node.attrs.from;
                ...
                processingMutex.mutex(async () => {
                    ...
                    if (msg.messageStubType === WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT) {
                        ...
                        if ((_b = msg.key.participant) === null || _b === void 0 ? void 0 : _b.endsWith('@lid')) {
                            const pn = node.attrs.participant_pn ||
                                (global._lidMap && global._lidMap[msg.key.participant]) ||
                                authState.creds.me.id;
                            global._lidMap = global._lidMap || {};
                            global._lidMap[msg.key.participant] = pn;
                            global._lidMap[pn] = msg.key.participant;
                        ...
                        if (!(0, WABinary_1.isJidGroup)(msg.key.remoteJid) && (0, WABinary_1.isLidUser)(msg.key.remoteJid)) {
                            const pn = node.attrs.sender_pn || node.attrs.peer_recipient_pn ||
                                (global._lidMap && global._lidMap[msg.key.remoteJid]);
                            if (pn) {
                                global._lidMap = global._lidMap || {};
                                global._lidMap[msg.key.remoteJid] = pn;
                                global._lidMap[pn] = msg.key.remoteJid;
                                msg.key.remoteJidAlt = pn;
                            }
                        }
