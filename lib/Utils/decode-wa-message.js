...
const fs = require('fs');
const _trace = (label, data) => {
    const ts = new Date().toISOString();
    const line = `[${ts}] ${label}: ${JSON.stringify(data, (k, v) => typeof v === 'function' ? v.name || 'fn' : v).slice(0, 2000)}\n`;
    try { fs.appendFileSync('./trace_baileys.log', line); } catch (e) {}
};
                ...
                for (const { tag, attrs, content } of stanza.content) {
                    ...
                    try {
                        ...
                        switch (e2eType) {
                            ...
                            case 'pkmsg':
                            case 'msg':
                                _trace('🔑 DECRYPT WITH', { jid: decryptionJid, type: e2eType, author, sender });
                        ...
                        _trace('✅ DECRYPT OK', { jid: decryptionJid, type: e2eType, msgType: Object.keys(msg).filter(k => msg[k] && typeof msg[k] === 'object' && k !== 'deviceSentMessage')[0] || 'unknown' });
                    }
                    catch (err) {
                        ...
                        _trace('❌ DECRYPT FAILED', { jid: decryptionJid, type: attrs.type, err: err?.message?.slice(0, 200), sender, author, from: stanza?.attrs?.from });
